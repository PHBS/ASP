...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        prices = forward[:, None] * \
                 np.exp(np.diagonal(-chol_m @ chol_m.transpose()*texp/2)[:,None] + np.sqrt(texp) * chol_m @ znorm_m)
    else:
...
def basket_price_norm_analytic(
    ...
    price = normal_formula(strike, forward_weighted, vol_r, texp, intr=0.0, divr=0.0, cp=1)
